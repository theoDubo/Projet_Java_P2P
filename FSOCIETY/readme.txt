Ce document est produit dans le cadre de la formation STRI à l'école d'ingénieurs UPSSITECH de Toulouse en France. 
...
depuis le dossier racine (ex : nous avons un dossier Serveur nous tapons alors ./Serveur/). 
    - il est ensuite demandé le port sur lequel on souhaite se connecter. Les connexions se font en localHost mais avec quelques ajustements du code il est possible
de recevoir des demandes de machines distantes. 
    - réexécuter un userPPP si vous souhaiter discuter avec une autre machine.
    - vous pouvez à  partir de maintenant executer plusieurs demandes telles que get, ls, ou un changement de racine. 

L'avenir possible ******************************************************************************

A l'heure actuelle le code ne permet pas d'accéder à  des machines distantes, une future amélioration peut être envisagée. 
La mise à jour n'est pas effectuée en cas d'ajout ou de suppression de fichiers sur les serveurs.
