...
public class Client_TCP implements Client,Runnable {
	...
	TimerTask timTas=new TimerTask() {
		...
		public void run() {
			...
			try {
				...
				os.println("maj");
				// Tant que l'on ne receptionne pas d'entier egal � -1
	...
	// Constructeur prenant en parametre un entier et un scanner
	public Client_TCP(int s,Scanner sc) {
		this.scanner = sc;
		this.sockServer=s;
	}
	...
	// Getter permettant de retourner une map deja classee qui contient les paquet allant de packetSelected1 � packetSelected2 et leurs tableaux de bytes associes
	...
	private void changeRacine(String s) throws IOException {
		PrintStream ps = new PrintStream(proxy.getOutputStream());
		//obligatoire due a notre structure
		Socket ss=new Socket("localhost",sockServer);
		PrintStream es = new PrintStream(ss.getOutputStream());
		//signaler le changement de route au proxy
		ps.println("ChangeRoot "+sockServer);
		//signaler la nouvelle route au serveur
		es.println("ChangeRoot "+s);
		es.close();
		ss.close();
	}
	...
	public void run() {
		...
		try {
			...
			// On ouvre le menu et le timer avec repetition toutes les minutes
